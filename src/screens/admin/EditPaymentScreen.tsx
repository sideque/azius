...
import { CustomButton, CustomInput, DatePickerField, Dropdown, useToast } from '../../components';
...
export function EditPaymentScreen() {
  ...
  const [payment, setPayment] = useState<Payment & { shopName: string } | null>(null);
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} keyboardShouldPersistTaps="handled">
      {payment && (
        <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Text style={{ color: colors.text, fontSize: 16, fontWeight: '600' }}>{payment.shopName}</Text>
          <Text style={{ color: colors.textSecondary, marginTop: 8 }}>Receipt Number</Text>
          <Text style={{ color: colors.text, fontSize: 14 }}>{payment.receiptNumber}</Text>
        </View>
      )}
      <CustomInput label="Payment Amount (₹)" value={amount} onChangeText={setAmount} keyboardType="decimal-pad" />
      <Dropdown
      ...
      <CustomButton title="Update Payment" onPress={handleSave} loading={saving} />
    </ScrollView>
  );
}
