...
export function EditSaleScreen() {
  ...
  const updateQuantity = (id: string, qty: number) => {
    setItems(items.map(i => i.id === id ? { ...i, quantity: qty, total: qty * i.rate } : i));
  };
  ...
  const handleSave = async () => {
    ...
    const result = await dispatch(updateSale({
      id: saleId,
      data: {
        items,
        subtotal,
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} keyboardShouldPersistTaps="handled">
      {sale && (
        <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <Text style={{ color: colors.text, fontSize: 16, fontWeight: '600' }}>{sale.shopName}</Text>
          <Text style={{ color: colors.textSecondary, marginTop: 8 }}>Invoice Number</Text>
          <Text style={{ color: colors.text, fontSize: 14 }}>{sale.invoiceNumber}</Text>
        </View>
      )}
      ...
      {items.map(item => (
        <View key={item.id} style={[styles.itemCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: colors.textSecondary }}>₹{item.rate} each</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Pressable onPress={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} style={[styles.qtyBtn, { backgroundColor: colors.primaryLight }]}>
              <Text style={{ color: colors.primary, fontSize: 18 }}>-</Text>
            </Pressable>
            <Text style={{ marginHorizontal: 12, color: colors.text, fontSize: 16 }}>{item.quantity}</Text>
            <Pressable onPress={() => updateQuantity(item.id, item.quantity + 1)} style={[styles.qtyBtn, { backgroundColor: colors.primaryLight }]}>
              <Text style={{ color: colors.primary, fontSize: 18 }}>+</Text>
            </Pressable>
            <Pressable onPress={() => removeItem(item.id)} style={[styles.qtyBtn, { backgroundColor: colors.error + '20', marginLeft: 16 }]}>
      ...
      <View style={[styles.summary, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <CustomInput label="Discount (₹)" value={discount} onChangeText={setDiscount} keyboardType="decimal-pad" />
        <View style={[styles.summaryRow, { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 12 }]}>
          <Text style={{ color: colors.text, fontWeight: '700', fontSize: 18 }}>Grand Total</Text>
          <Text style={{ color: colors.primary, fontWeight: '700', fontSize: 18 }}>₹{grandTotal.toFixed(2)}</Text>
        </View>
      </View>

      <CustomButton title="Update Sale" onPress={handleSave} loading={saving} style={{ marginTop: 16, marginBottom: 40 }} />
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  itemCard: { flexDirection: 'row', padding: 12, borderRadius: 8, borderWidth: 1, marginBottom: 8, alignItems: 'center' },
