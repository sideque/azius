...
export async function createSale(
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const totalCost = items.reduce(
      (sum, item) =>
        sum + Number(item.purchasePrice) * Number(item.quantity),
      0,
    );
  ...
  const totalCost = saleItemsWithName.reduce(
    (sum, item) =>
      sum +
      (productSnapshotMap[item.productId]?.purchasePrice ?? 0) *
        item.quantity,
    0,
  );
...
export async function getLowStockProducts(
  ...
  const snapshot = await getDocs(productsCollection);
  const products = snapshot.docs.map(mapDoc<Product>);
  return products.filter((p) => (Number(p.stockQuantity) || 0) <= threshold);
}

// Shops with high outstanding balance
export async function getShopsWithHighOutstanding(
  threshold: number,
): Promise<Shop[]> {
  const snapshot = await getDocs(shopsCollection);
  const shops = snapshot.docs.map(mapDoc<Shop>);
  return shops.filter((s) => (Number(s.outstandingBalance) || 0) >= threshold);
}
...
export async function exportAllData(): Promise<string> {
  const [
    ...
    getDocs(shopsCollection),
    getDocs(suppliersCollection),
    getDocs(salesCollection),
    getDocs(saleItemsCollection),
    getDocs(paymentsCollection),
    getDocs(supplierPaymentsCollection),
    getDocs(supplierBillsCollection),
    getDocs(ledgersCollection),
    getDocs(expensesCollection),
  ]);

  return JSON.stringify({
    users: users.docs.map(mapDoc),
    products: products.docs.map(mapDoc),
    shops: shops.docs.map(mapDoc),
    suppliers: suppliers.docs.map(mapDoc),
    sales: sales.docs.map(mapDoc),
    saleItems: saleItems.docs.map(mapDoc),
    payments: payments.docs.map(mapDoc),
    supplierPayments: supplierPayments.docs.map(mapDoc),
    supplierBills: supplierBills.docs.map(mapDoc),
    ledgers: ledgers.docs.map(mapDoc),
    expenses: expenses.docs.map(mapDoc),
