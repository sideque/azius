...
export const fetchNotifications = createAsyncThunk('settings/notifications', async () => {
  ...
  highOutstanding.forEach((s) => {
    notifications.push({
      ...
      message: `${s.shopName} has outstanding balance of ₹${s.outstandingBalance.toLocaleString()}`,
      createdAt: now,
      read: false,
    });
...
const settingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(backupDatabase.pending, (state) => { state.loading = true; })
      .addCase(backupDatabase.fulfilled, (state, action) => { state.loading = false; state.backupData = action.payload; })
...
export const { markNotificationRead, markAllRead, clearError } = settingsSlice.actions;
