...
export function validateNumber(value: string, fieldName: string, min = 0): string | null {
...
export function validateProduct(data: Record<string, string>): ValidationResult {
  const errors: Record<string, string> = {};
  ...
  ['purchasePrice', 'sellingPrice', 'stockQuantity'].forEach((key) => {
    ...
    const err = validateNumber(data[key] ?? '', label.charAt(0).toUpperCase() + label.slice(1));
    if (err) errors[key] = err;
  });
  if (!errors.sellingPrice && !errors.purchasePrice) {
    ...
    if (sell < purchase) errors.sellingPrice = 'Selling price should be >= purchase price';
  }
  return { isValid: Object.keys(errors).length === 0, errors };
}
...
export function validateShop(data: Record<string, string>): ValidationResult {
  ...
  ['shopName', 'ownerName', 'address'].forEach((key) => {
    const label = key.replace(/([A-Z])/g, ' $1').trim();
    const err = validateRequired(data[key] ?? '', label.charAt(0).toUpperCase() + label.slice(1));
    if (err) errors[key] = err;
  });
...
export function validatePayment(amount: string, shopId: string): ValidationResult {
  const errors: Record<string, string> = {};
